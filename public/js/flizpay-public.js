...
jQuery(function ($) {
  $(document).ready(function () {
    ...
    const waitLabel = navigator.language.includes("en")
      ? "Wait..."
      : "Warten...";
    const redirectInfo = navigator.language.includes("en")
      ? "You will be redirected to the FLIZpay safe checkout."
      : "Sie werden zum sicheren Checkout von FLIZpay weitergeleitet.";
    const redirectButtonLabel = navigator.language.includes("en")
      ? "Click here to pay now"
      : "Klicken Sie hier, um jetzt zu bezahlen";
    const paymentCompleteLabel = navigator.language.includes("en")
      ? "Payment Complete! Finishing Order..."
      : "Zahlung abgeschlossen! Bestellung wird abgeschlossen...";
    ...
    const FLIZ_LOADING_HTML = `
      <div style="font-size: 0.9rem; padding: 8px; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center;">
        ...
        <button 
          ...
          id="flizpay-redirect-button">
          ${redirectButtonLabel}
        </button>
        <button 
          style="cursor: pointer; text-decoration: underline; border: none; color: black; background-color: white; margin-top: 16px;"
    ...
    function bind_blocks_hook() {
      const unsubscribe = wp.data.subscribe(() => {
        ...
        if (
          ...
          const redirectUrl = wp.data
            .select("wc/store/payment")
            .getPaymentResult().paymentDetails.checkout_page;
          ...
          if (settings.url.indexOf("wc-ajax=checkout") !== -1) {
            ...
            if (response && response.result === "success") {
              if (response.order_id) {
                checkoutUrl = response.checkout_page;
                stopPolling = false;
                fliz_block_ui();
      ...
      setTimeout(() => {
        jQuery.blockUI({
          ...
          onBlock: () => {
            ...
            const flizRedirectButton = document.querySelector(
              "#flizpay-redirect-button"
            );
            flizRedirectButton.onclick = () => {
        ...
        success: function (response) {
          setTimeout(function () {
            ...
            if (
              ...
              jQuery(flizCancelButton).hide();
              jQuery(flizRedirectButton).hide();
              document.querySelector("#flizpay-redirect-info").innerHTML =
